package com.alibaba.datax.plugin.reader.hdfsreaderv1;
...
public class DFSUtil {
    private static final Logger LOG = LoggerFactory.getLogger(HdfsReaderV1.Job.class);
    ...
    public DFSUtil(Configuration taskConfig) {
        ...
        String hadoopConfDir = StringUtils.trimToNull(System.getenv("HADOOP_CONF_DIR"));
        if (!StringUtils.isBlank(hadoopConfDir)) {
            File file = new File(hadoopConfDir + "/core-site.xml");
            if (file.exists()) {
    ...
    public void parquetFileStartRead(String sourceOrcFilePath, Configuration readerSliceConfig,
        ...
        // 判断是否读取所有列
        if (null == column || column.size() == 0) {
            int allColumnsCount = getParquetAllColumnsCount(sourceOrcFilePath);
            columnIndexMax = allColumnsCount - 1;
            isReadAllColumns = true;
        } else {
            columnIndexMax = getMaxIndex(column);
    ...
    private int getParquetAllColumnsCount(String filePath) {
        ...
        try {
            ...
            columnsCount = metadata.getFileMetaData().getSchema().getColumns().size();
            return columnsCount;
        } catch (IOException e) {
    ...
    public boolean checkHdfsFileType(String filepath, String specifiedFileType) {
        ...
        try {
            ...
            if (StringUtils.equalsIgnoreCase(specifiedFileType, Constant.CSV)
                ...
                boolean isPAR = isPARFile(file, fs, in);// 判断是否是 Sequence File
                if (isPAR) {
                    return false;
                }
                // 如果不是ORC,RC,PAR和SEQ,则默认为是TEXT或CSV类型
    ...
    // 判断file是否是PAR File
    private boolean isPARFile(Path file, FileSystem fs, FSDataInputStream in) {
        try {
           final byte[] PAR_MAGIC = new byte[] {(byte) 'P', (byte) 'A', (byte) 'R'};
           byte[] magic = new byte[PAR_MAGIC.length];
           in.seek(0);
           in.readFully(magic);
           return Arrays.equals(magic, PAR_MAGIC);
        } catch (IOException e) {
            LOG.info(String.format("检查文件类型: [%s] 不是ORC File.", file.toString()));
        }
        return false;
