package com.alibaba.datax.plugin.writer.hdfswriterv1;
...
public class HdfsWriterV1 extends Writer {
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            if(fileType.equalsIgnoreCase("TEXT")){
            ...
            }else if(fileType.equalsIgnoreCase("ORC") || fileType.equalsIgnoreCase("PAR")){
                ...
                if(null == compress){
                    ...
                    if(!orcSupportedCompress.contains(compress)){
                        throw DataXException.asDataXException(HdfsWriterErrorCode.ILLEGAL_VALUE,
                                String.format("目前ORC/PAR FILE仅支持SNAPPY压缩, 不支持您配置的 compress 模式 : [%s]",
                                        compress));
                    }
                }
        ...
        public void prepare() {
            ...
            if(hdfsHelper.isPathexists(path)){
                if(!hdfsHelper.isPathDir(path)){
                    ...
                    if (this.defaultFS.toUpperCase().startsWith("S3")) {
                        LOG.warn(String.format("您配置的S3 path: [%s] 不存在，将创建", path));
                        if (!hdfsHelper.createPath(path)) {
                            throw DataXException.asDataXException(HdfsWriterErrorCode.CREATE_S3_PATH_ERROR,
                                    String.format("创建S3 path: [%s] 失败, 可以先创建S3路径.", path));
                        }
                    } else if (this.defaultFS.toUpperCase().startsWith("OSS")) {
                        LOG.warn(String.format("您配置的OSS path: [%s] 不存在，将创建", path));
                        if (!hdfsHelper.createPath(path)) {
                            throw DataXException.asDataXException(HdfsWriterErrorCode.CREATE_OSS_PATH_ERROR,
                                    String.format("创建OSS path: [%s] 失败, 可以先创建OSS路径.", path));
                        }
                    } else {
                        throw DataXException.asDataXException(HdfsWriterErrorCode.ILLEGAL_VALUE,
                                String.format("您配置的path: [%s] 不存在, 清先创建.", path));
                    }
                }
